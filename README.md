...
- **Handheld OCR policy:** The app samples the live image to wait for a briefly stable card, then attempts OCR no more than about once every 0.75 seconds. A high-confidence, unambiguous species read acts immediately—important when a bulk card is visible for only 1–2 seconds. Medium-confidence candidates need two agreeing reads within roughly 3.2 seconds, and an unreadable intervening frame does not erase that recent evidence.
...
3. Hold the card still briefly. A high-confidence, unambiguous read can alert after one accepted stable OCR pass; medium reads gather repeat evidence over a short handheld window.
